...
//	gas.SetComposition("deuterium", 100.);
//	gas.SetPressure(600.);
//	gas.SetTemperature(293.15);
//	gas.SetFieldGrid(10., 1000., 20, true, 3., 3., 1, -0.1, 0.1, 5);
//	const int ncoll = 10;
//	gas.GenerateGasTable(ncoll);
//	gas.WriteGasFile("deuterium_600.gas");

	gas.LoadGasFile("deuterium_600.gas");
...
//  gas.LoadIonMobility(path + "/share/Garfield/Data/IonMobility_Ar+_Ar.txt");
  ...
  const double vCathode = -60000;
  // Voltage on Anode [V]
  const double vAnode = -3020;
  // Voltage on first ring [v]
  const double vFirst = -3020;
  // Add rows of rings
  int i, j;
  double volts, xcoord;
  for (i=0; i<nRing; i++) {
...
//  fclose(ffile);
  ...
  sole.SetMagneticField(3, 0, 0);
    ...
    TCanvas* cf = new TCanvas("cf", "hi", 1200, 600);
